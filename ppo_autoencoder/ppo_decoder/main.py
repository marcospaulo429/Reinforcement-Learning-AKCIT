...
# Função de inicialização de camadas
...
class VAE(nn.Module):
    ...
    def forward(self, x):
        mu, logvar = self.encoder(x)  # Encoder
...
if __name__ == "__main__":
    ...
    args.recon_coef = random_search()

    for i in range(args.num_random_search):
        run_name = f"decod_{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}"
        if args.track:
            import wandb
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            for epoch in range(args.update_epochs):
                ...
                # ✅ Aqui está o ponto CERTO para salvar imagens 1 vez por epoch
            if (iteration) % 10 == 0:
                print("Saving images...")
            ...
            writer.add_scalar("losses/recons_loss", recon_loss, global_step)
